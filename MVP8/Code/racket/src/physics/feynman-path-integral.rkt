...
(define (compute-action steps)
  (if (null? steps)
      ...
      (foldl abstract-mul (get-one) (map (λ (step)
                          (define from-val (strip-tags-deep (semiring-element-value (micro-step-from-term step))))
                          (define to-val (strip-tags-deep (semiring-element-value (micro-step-to-term step))))
...
(define (two-point-correlation histories x-term y-term)
  ...
  (define correlations (map (λ (weight)
                              (abstract-complex-mul weight (abstract-complex-mul (abstract-complex (strip-tags-deep (semiring-element-value x-term)) (get-zero))
                                                                                     (abstract-complex (strip-tags-deep (semiring-element-value y-term)) (get-zero)))))
                            weights))
...
(define (strip-tags-deep v)
  (cond
    [(abstract-op? v)
     (define op (abstract-op-operator v))
     (define args (abstract-op-operands v))
     (case op
       [(OriginTag) (strip-tags-deep (second args))]
       [(add)
        (abstract-add (strip-tags-deep (first args))
                      (strip-tags-deep (second args)))]
       [(mul)
        (abstract-mul (strip-tags-deep (first args))
                      (strip-tags-deep (second args)))]
       [(sub)
        (abstract-sub (strip-tags-deep (first args))
                      (strip-tags-deep (second args)))]
       [(abs)
        (abstract-abs (strip-tags-deep (first args)))]
       [else v])]
    [else v]))
