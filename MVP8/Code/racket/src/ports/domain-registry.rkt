...
;; Professional wrappers and run utilities
(struct domain-port-result (ok? value error timings meta) #:transparent)

(define (timed thunk)
  (define start (current-inexact-milliseconds))
  (with-handlers ([exn:fail? (λ (e)
                               (values #f (/ (- (current-inexact-milliseconds) start) 1000.0) e))])
    (define v (thunk))
    (values v (/ (- (current-inexact-milliseconds) start) 1000.0) #f)))

(define (wrap-encoder enc)
  (λ (input)
    (define in*
      (cond
        [(and (semiring-element? input)
              (eq? (semiring-element-semiring-type input) B))
         (reconstitute input)]
        [else input]))
    (enc in*)))

(define (wrap-normalizer norm)
  (λ (ev)
    (define out (norm ev))
    (cond
      [(and (semiring-element? out)
            (eq? (semiring-element-semiring-type out) B))
       (B-normalize out)]
      [else out])))

(define (wrap-decoder dec)
  (λ (norm)
    (define v (dec norm))
    (cond
      [(semiring-element? v) v]
      [else (semiring-element v Core)])))

(define (wrap-port-professional port)
  (match port
    [(struct domain-port (name encoder evaluator normalizer decoder carrier totality-predicate q-vector))
     (domain-port name (wrap-encoder encoder) evaluator (wrap-normalizer normalizer) (wrap-decoder decoder)
                  carrier totality-predicate q-vector)]
    [_ port]))

(define (domain-port-run port input)
  (match port
    [(struct domain-port (name encoder evaluator normalizer decoder carrier totality-predicate q-vector))
     (define meta (hash 'name name 'carrier carrier 'q-vector q-vector))
     (with-handlers ([exn:fail? (λ (e)
                                  (domain-port-result #f 'undefined e '() meta))])
       (if (not (totality-predicate input))
           (domain-port-result #f 'undefined 'partial '() meta)
           (let-values ([(enc enc-t err1) (timed (λ () (encoder input)))])
             (if err1
                 (domain-port-result #f 'undefined err1 (list (cons 'encode enc-t)) meta)
                 (let-values ([(ev ev-t err2) (timed (λ () (evaluator enc)))])
                   (if err2
                       (domain-port-result #f 'undefined err2 (list (cons 'encode enc-t) (cons 'eval ev-t)) meta)
                       (let-values ([(norm norm-t err3) (timed (λ () (normalizer ev)))])
                         (if err3
                             (domain-port-result #f 'undefined err3 (list (cons 'encode enc-t)
                                                                          (cons 'eval ev-t)
                                                                          (cons 'norm norm-t))
                                                 meta)
                             (let-values ([(dec dec-t err4) (timed (λ () (decoder norm)))])
                               (if err4
                                   (domain-port-result #f 'undefined err4 (list (cons 'encode enc-t)
                                                                                (cons 'eval ev-t)
                                                                                (cons 'norm norm-t)
                                                                                (cons 'decode dec-t))
                                                       meta)
                                   (domain-port-result #t dec #f (list (cons 'encode enc-t)
                                                                       (cons 'eval ev-t)
                                                                       (cons 'norm norm-t)
                                                                       (cons 'decode dec-t))
                                                      meta))))))))))) ]
    [_ (domain-port-result #f 'undefined (exn:fail:contract "invalid-port" (current-continuation-marks)) '() (hash))]))

(define (port-verbose?)
  (let ([v (getenv "LUX_PORT_VERBOSE")]) (and v (or (string-ci=? v "1") (string-ci=? v "true")))))

(define (domain-port-run/print port input)
  (define res (domain-port-run port input))
  (when (port-verbose?)
    (printf "port ~a → ok=~a timings=~a\n" (domain-port-name port) (domain-port-result-ok? res) (domain-port-result-timings res)))
  res)
...
(define (get-domain-port name)
  (match name
    ['turing (wrap-port-professional (make-turing-port))]
    ['lambda (wrap-port-professional (make-lambda-port))]
    ['blockchain (wrap-port-professional (make-blockchain-port))]
    ['qft (wrap-port-professional (make-qft-port))]
    ['feynman (wrap-port-professional (make-qft-port))]
    ['proof-assistant (wrap-port-professional (make-proof-assistant-port))]
    ['analytic (wrap-port-professional (make-analytic-number-theory-port))]
    ['analytic-number-theory (wrap-port-professional (make-analytic-number-theory-port))]
    ['category (wrap-port-professional (make-categorical-logic-port))]
    ['category-theory (wrap-port-professional (make-categorical-logic-port))]
    ['categorical-logic (wrap-port-professional (make-categorical-logic-port))]
    ['generating-functionals (wrap-port-professional (make-generating-functionals-port))]
    ['paradigms (wrap-port-professional (make-computational-paradigms-port))]
    ['comp-paradigms (wrap-port-professional (make-computational-paradigms-port))]
    ['complexity-separation (wrap-port-professional (make-complexity-separation-port))]
    ['complexity (wrap-port-professional (make-complexity-separation-port))]
    ['p-np (wrap-port-professional (make-complexity-separation-port))]
    [_ #f]))

;; Convenience: run a port with professional pipeline and timing
(define (run-port name input)
  (define p (get-domain-port name))
  (and p (domain-port-run/print p input)))
