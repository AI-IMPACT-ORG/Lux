...
(require rackunit
         ...
         (file "../foundation/lux-axioms.rkt")
...
(define (heavy-checks)
  (list
   ...
   (cons 'smatrix-analytic (λ () ((dynamic-require (build-path here "../ports/qft/analytic-smatrix.rkt") 'smatrix-analytic-pack))))
   (cons 'amplituhedron (λ () ((dynamic-require (build-path here "../ports/qft/amplituhedron.rkt") 'amplituhedron-pack))))
   ;; Additional thorough checks
   (cons 'info-metric (λ () ((dynamic-require (build-path here "../theorems/info-metric.rkt") 'info-metric-pack))))
   (cons 'smatrix (λ () ((dynamic-require (build-path here "../ports/qft/smatrix.rkt") 'smatrix-pack))))
   (cons 'traced-operators (λ () ((dynamic-require (build-path here "../logic/traced-operators.rkt") 'traced-operators-pack))))
   (cons 'hp-from-rg (λ () ((dynamic-require (build-path here "../ports/analytic-number-theory/hp-from-rg.rkt") 'hp-from-rg-pack))))
   (cons 'gf-constraints (λ () ((dynamic-require (build-path here "../theorems/generating-functionals.rkt") 'verify-generating-functional-constraints))))))

(define (verify-all)
  (displayln "=== Lux verification: axioms → consequences ===")
  ;; Start proof registry and load LaTeX labels, then verify Lux Axioms runner
  (start-registry!)
  (load-latex-labels!)
  ;; Verify Lux Axioms runner completes without failures
