...
(command-line
  ...
  ["--heavy-json" path "Write heavy-scan results as JSON to <path>"
   (set! heavy-json-path path)]
...
(define (run-heavy-scan)
  ...
  (for ([kv (in-list (heavy-checks))])
    ...
    (define secs (cdr res))
    (set! results (append results (list (list name status secs))))
    ...
    (when (or (eq? status 'fail) (eq? status 'timeout))
      ...
      (when heavy-json-path
        (call-with-output-file heavy-json-path
          (λ (out)
            (define j (for/list ([tr (in-list results)])
                        (hash 'name (symbol->string (first tr))
                              'status (symbol->string (second tr))
                              'elapsed (third tr))))
            (displayln (jsexpr->string j) out))
          #:exists 'truncate))
      (exit 1)))
  (newline)
