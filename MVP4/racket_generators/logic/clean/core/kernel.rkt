...
;; Zero header constant (now using header struct)
(define kernel-header-zero header-zero)

;; Header operations (now using header struct methods)
(define (kernel-header-add h1 h2)
  (header-add h1 h2))

(define (kernel-header-product h1 h2)
  (header-multiply h1 h2))
...
;; RG blocking using header struct (more precise)
(define (rg-block-kernel kernel block-size)
  "Apply RG blocking to a kernel with block size k using header struct"
  (define blocked-header (rg-block (kernel-header kernel) block-size))
  (kernel blocked-header (kernel-transition kernel)))
...
(define (random-kernel [header-range 5.0])
  ...
  (define transition-name (string->symbol (format "random-~a" (random 1000))))
  (kernel h (transition transition-name (λ (term) term) '())))
