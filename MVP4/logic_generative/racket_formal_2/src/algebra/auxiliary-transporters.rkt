...
(define (starB b)
  (define d (dec-z-zbar b))
  (rec-z-zbar (first d) (third d) (second d) (fourth d)))

(define (monoid-semiring-multiply b1 b2)
  (define d1 (dec-z-zbar b1))
  (define d2 (dec-z-zbar b2))
  (define k (+ (first d1) (first d2)))
  (define mz (+ (second d1) (second d2)))
  (define mzb (+ (third d1) (third d2)))
  (define core ((semiring-ops-mul Core-ops) (fourth d1) (fourth d2)))
  (rec-z-zbar k mz mzb core))
